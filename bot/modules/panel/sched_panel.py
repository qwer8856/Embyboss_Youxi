...
# Init commands and restart check after startup
...
async def sched_change_policy(_, call):
    try:
        ...
        save_config()
        await asyncio.gather(callAnswer(call, f"OK {method} changed"), sched_panel(_, call.message))
    except IndexError:
...
async def check_ex_admin(_, msg):
    ...
    try:
        ...
        pass
    if confirm == "true":
        send = await msg.reply("Running expiration check...")
        await asyncio.gather(check_expired(), send.edit("Expiration check done."))
    else:
        await msg.reply("Please run `/check_ex true`")
...
async def run_low_ac(_, msg):
    ...
    try:
        ...
        pass
    if confirm == "true":
        send = await msg.reply("Running low-activity check...")
        await asyncio.gather(check_low_activity(), send.delete())
    else:
        await msg.reply("Please run `/low_activity true`")
...
async def shou_dong_uplayrank(_, msg):
    ...
    try:
        ...
        await msg.reply(
            f"Please run `/uranks <days>`.\n"
            f"This manual run does not affect settlement of {sakura_b}.\n"
            f"Timed settlement status: {_open.uplays}"
        )
...
async def page_uplayrank(_, call):
    ...
    await callAnswer(call, f"Go to page {j}")
    a, b, _ = await Uplaysinfo.users_playback_list(days)
    if not a:
        return await callAnswer(call, f"Failed to fetch {days} days user plays.", True)
...
async def update_bot(force: bool = False, msg: Message = None, manual: bool = False):
    ...
    if resp.status_code == 200:
        ...
        if latest_commit != auto_update.commit_sha:
            ...
            text = "[AutoUpdate_Bot] Update success, restarting..."
            if not msg:
            ...
            message = "[AutoUpdate_Bot] No updates found."
            await bot.send_message(chat_id=group[0], text=message) if not msg else await msg.edit(message)
            LOGGER.info(message)
    else:
        text = "[AutoUpdate_Bot] Failed, please check git_repo. Example: qwer8856/Embyboss_Youxi"
...
async def get_update_bot(_, msg: Message):
    ...
    send_task = bot.send_message(chat_id=msg.chat.id, text="Updating bot code, please wait...")
